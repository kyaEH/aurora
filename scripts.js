...
$("input").change(function() {
	...
	Agilité = Number($("#agilite").val())
	Intelligence = Number($("#intelligence").val())
	Concentration = Number($("#concentration").val())
	Dextérité = Number($("#dexterite").val())
	// precision = agilité/2.75 + dexterité/2 
	$("#precision").val(Math.min(18, 9 + Math.round(Agilité / 3 + Dextérité / 2 + Number($("#lunette").val()))));
	//dégats = force + arme1 + arme2
	$("#degats").val(Math.round(Force*1 + Number($("#arme1").val())*1+Number($("#arme2").val())*1));
	//magie = intelligence/1.25 + concentration/2 + amulette
	$("#magie").val(Math.round(Intelligence / 1.25 + Concentration / 2 + Number($("#amulette").val())*1));
	// critique = force/2 + agilité/2
	$("#crit").val(Math.round(Force / 2 + Agilité / 2+Concentration*0.25));
	//  buff&debuff = concentration/2 + dextérité/2
	$("#buffdebuff").val(Math.min(17, 2+Math.round(Concentration / 2 + Dextérité / 2)));
	// parade = constitution/2.5 + armure
	$("#parade").val(Math.round(Constitution / 2.5 + Number($("#armure").val())*1));
	// esquive = agilité + age + taille + poids
	$("#esquive").val(Math.max(10,Math.min(75, Math.round(Agilité + 90 - (Number($("#age").val()) / 2 + Number($("#taille").val()) / 10 + Number($("#poids").val()) / 5)))));
	
    // --- Social ---
	$("#niveau").val(Math.floor(Math.max(1, Math.sqrt(Number($("#experience").val()) / 5))) - 1);
    //rapidite = agilite * 1.5 + intelligence * 1.5 + bottes
	$("#rapidite").val(Math.floor(5+Agilité*1.5+Intelligence*0.75 + Number($("#bottes").val())*1 + (10-Number($("#taille").val())/20)));
    // /r @abilities.cha.value*1.5 + @abilities.int.value +  @equipement.Chaussure.value + (10-@attributes.taille.value/20)
	//furtivite = agilite/2 + concentration/3 + 10-taille/10 
	$("#furtivite").val(Math.min(17,Math.max(4,Math.round(3+Math.floor(Agilité/2 + Concentration/2.5)))));
    //perception= intelligence / 2 + concentration / 2
	$("#perception").val(Math.min(17,Math.floor(8+Intelligence/2 + Concentration*0.75)));
    //mana= intelligence *1.25 + dexterite *0.75
	$("#mana").val(Math.floor(2+Intelligence*1.25+Dextérité*0.75));
    //pvmax= force * 1.5 + constition * 2
	$("#pvmax").val(Math.floor(10+Force*1.25+ Constitution*2 + Number($("#poids").val()) / 15));
    //charisme = constitution/2 + intelligence/1.5 + charme
	$("#charisme").val(Math.min(17, 5+Math.floor(Constitution/1.75+Intelligence/1.75+Number($("#charme").val())*1+Number($("#taille").val()) / 75)));

	$("#remaining").val(16+Number($("#niveau").val())-(Force+Constitution+Agilité+Intelligence+Concentration+Dextérité));	
...
function exportData() {
	...
	if (window.confirm("Voulez vous enregistrer la fiche de "+$('#charname').val()+" en fichier?\n(Le lien de la fiche sera aussi enregistrée dans le presse papier)")) {
		...
		$.ajax({
			...
			var request = new XMLHttpRequest();
			request.open("POST", b64d("aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTE3MTExNDU5NTE3MjIzNzMzMi9vdGtmbnFXWElGWnN2ZlRJSENFSUtpeFUwcDJkRmNkdWFEUDhUWGs0MDFJUV9zTy10THI0MnJWX016d2w3TElBVG1HMw=="));
			request.setRequestHeader('Content-type', 'application/json');
			var params = {
				username: "Autosave",
...
$(document).ready(function() {
	...
	Swal.fire({
		title: "Information concernant le Faisceau d'Aurora",
		text: "En raison du nouveau système implémenté, le site web de la fiche n'est plus maintenu. Vous pouvez toujours l'utiliser, mais il est recommandé d'utiliser le nouveau système de jeu. Pour plus d'informations, veuillez consulter le serveur Discord de la communauté du Faisceau d'Aurora.",
		icon: "info",
		footer: '<a href="https://discord.com/invite/FVHnKQkGJ2">Rejoindre le Discord</a>'
	  });
